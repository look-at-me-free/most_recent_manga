(() => {
  ...
  function isMappedWork(work) {
    return !!(work && work.use_map === true);
  }

  function getMapFileName(work) {
    return String(work?.map_file || "").trim().replace(/^\/+/, "");
  }

  function getMapJsonUrl(work) {
    const file = getMapFileName(work);
    if (!file) {
      throw new Error(`Mapped work "${work?.slug || "(unknown)"}" is missing map_file.`);
    }
    return `${normalizeBaseUrl(CONFIG.mapsBase)}/${file}`;
  }

  async function loadMapsModule() {
    if (STATE.mapsModulePromise) return STATE.mapsModulePromise;
    STATE.mapsModulePromise = import(CONFIG.mapsModule);
    return STATE.mapsModulePromise;
  }
  ...
  function getFirstEntry() {
    for (const work of STATE.works) {
      ...
      if (work?.slug && isMappedWork(work)) return { work, entry: first || null };
    }
  ...
  function resolveSelection(dir, file) {
    ...
    for (const work of STATE.works) {
      ...
      if (isMappedWork(work)) {
        return { work, entry: entries[0] || null };
      }
    ...
    if (matchedWork && isMappedWork(matchedWork)) {
      const entries = Array.isArray(matchedWork.entries) ? matchedWork.entries : [];
      return { work: matchedWork, entry: entries[0] || null };
    }
  ...
  function renderWorksNav() {
    ...
    if (STATE.isMobileReader) {
      ...
      for (const work of STATE.works.filter(w => w.top_pill !== false)) {
        ...
        const entries = Array.isArray(work.entries) ? work.entries : [];
        const countLabel = entries.length ? `${entries.length} ${entries.length === 1 ? "chapter" : "chapters"}` : "open";
        ...
        } else if (isMappedWork(work)) {
          html += `
            <button class="mobile-chapter-link${isActiveWork ? " current" : ""}" type="button" data-dir="${escapeHtml(work.slug)}" data-file="">
              Open ${escapeHtml(work.display || titleCaseSlug(work.slug))}
            </button>
          `;
        }
    ...
    for (const work of STATE.works.filter(w => w.top_pill !== false)) {
      ...
      if (entries.length) {
        ...
        html += `
          <a href="?dir=${encodeURIComponent(work.slug)}&file=" class="topworks-link${isActive ? " active" : ""}" data-dir="${escapeHtml(work.slug)}" data-file="">
  ...
  function wireMobileWorksNav() {
    ...
    nav.addEventListener("click", async (e) => {
      ...
      const chapterBtn = e.target.closest("button[data-dir]");
  ...
  function buildDesktopTopTraversal(entries, currentIndex, prev, next) {
    ...
    const currentLabel = STATE.currentEntry?.subtitle || titleCaseSlug(STATE.currentEntry?.slug || "Current");
  ...
  function updateChapterProgress(progress = 0) {
    ...
    if (label) {
      const chapterNo = getCurrentChapterPosition();
      const baseLabel = STATE.currentEntry?.subtitle || STATE.currentItem?.subtitle || STATE.currentWork?.display || "Chapter Progress";
  ...
  function maybePreloadNextChapter() {
    ...
    if (isMappedWork(STATE.currentWork)) return;
  ...
  // ------------------------------------------------------------
  // Mapped work delegation
  // ------------------------------------------------------------

  async function buildMappedReader(resolved) {
    const reader = document.getElementById("reader");
    if (!reader) return;

    STATE.currentWork = resolved.work;
    STATE.currentEntry = resolved.entry || null;
    ...
    if (workTitleEl) {
      const entryLabel = STATE.currentEntry?.subtitle || STATE.currentEntry?.slug || "";
      workTitleEl.textContent = entryLabel
        ? `${resolved.work.display || titleCaseSlug(resolved.work.slug)} · ${entryLabel}`
        : `${resolved.work.display || titleCaseSlug(resolved.work.slug)}`;
    }
    ...
    const mapsModule = await loadMapsModule();
    if (!mapsModule || typeof mapsModule.renderMappedWork !== "function") {
      throw new Error(`maps.js must export an async function named renderMappedWork(ctx).`);
    }

    const mapUrl = getMapJsonUrl(resolved.work);

    await mapsModule.renderMappedWork({
      work: resolved.work,
      entry: resolved.entry || null,
      mapUrl,
      mapFile: getMapFileName(resolved.work),
      reader,
      state: STATE,
      config: CONFIG,
      zones: ZONES,
      specialZones: SPECIAL_ZONES,
      helpers: {
        $,
        $$,
        createEl,
        ...
        makeIns,
        makeSpecialIns,
        ensureAdProviderScript,
        setupAdVisibilityObserver,
        startRefreshTimers,
        clearRefreshTimers,
        clearDesktopAdShells,
        updateChapterProgress,
        updateStickyBottomAction,
        showRetentionToast,
        getEntryContext,
        getCurrentChapterPosition,
  ...
  function shouldShowInterstitial(dir, file, options = {}) {
    ...
    if (isMappedWork(selection.work)) return false;
  ...
  async function buildStandardReader(resolved) {
    const reader = document.getElementById("reader");
    if (!reader) return;
  ...
  async function buildReader() {
    ...
    if (!resolved) {
      ...
      resolved = first.work ? first : null;
      if (resolved) {
        setQueryState(resolved.work.slug, resolved.entry?.slug || "", true);
      }
    }

    if (!resolved || !resolved.work) {
      throw new Error("No works found in library.json");
    }

    if (isMappedWork(resolved.work)) {
      await buildMappedReader(resolved);
      return;
    }

    if (!resolved.entry) {
      throw new Error(`No entry found for work "${resolved.work.slug}".`);
    }

    await buildStandardReader(resolved);
  }
  ...
  async function switchEntry(dir, file, replace = false, options = {}) {
    ...
    showRetentionToast(
      `Now reading: ${
        ...
        STATE.currentEntry?.slug ||
        STATE.currentWork?.display ||
        titleCaseSlug(file || dir)
      }`
    );
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    boot().catch(err => {
      ...
      if (reader) {
        reader.innerHTML = `
          <div class="note">
            Failed to load this work. Please check library.json, map_file, maps.js, sources, item.json, base_url, and image filenames.
          </div>
        `;
      }
    });
